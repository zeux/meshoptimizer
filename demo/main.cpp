...
void encodeIndex(const Mesh& mesh)
{
	...
	buffer.resize(meshopt_encodeIndexBuffer(&buffer[0], buffer.size(), &mesh.indices[0], mesh.indices.size(), MESHOPTIMIZER_INDEX_CODEC_VERSION));

	clock_t middle = clock();

	std::vector<uint32_t> result(mesh.indices.size());
	meshopt_decodeIndexBuffer(&result[0], mesh.indices.size(), &buffer[0], buffer.size(), MESHOPTIMIZER_INDEX_CODEC_VERSION);
