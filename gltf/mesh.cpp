...
static void simplifyMesh(Mesh& mesh, float threshold, bool aggressive)
{
	...
	indices.resize(meshopt_simplify(
	    &indices[0], &mesh.indices[0], mesh.indices.size(), positions->data[0].f, vertex_count, sizeof(Attr), target_index_count, target_error));
	...
	{
		indices.resize(
		    meshopt_simplifySloppy(&indices[0], &mesh.indices[0], mesh.indices.size(), positions->data[0].f, vertex_count, sizeof(Attr), target_index_count));
		mesh.indices.swap(indices);
	}
}
...
void debugSimplify(const Mesh& source, Mesh& kinds, Mesh& loops, float ratio)
{
	...
	for (size_t i = 0; i < mesh.streams.size(); ++i)
	{
		...
		if (stream.target == 0 &&
		    (stream.type == cgltf_attribute_type_position || stream.type == cgltf_attribute_type_joints || stream.type == cgltf_attribute_type_weights))
		{
